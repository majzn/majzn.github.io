...
int main(void) {
  ...
  while (tb_update()) {
    /* Input Handling */
    if (tb_key_pressed(TB_KEY_ESCAPE)) { break; }
    ...
    for (y = 0; y < h; ++y) {
      for (x = 0; x < w; ++x) {
        ...
        if (((frame_counter * 2) / 256) % 2 == 1) { b = 255 - b; }
    ...
    /* 3. Text & Mouse Info Overlay */
    {
      ...
      if (w > 0) { frequency = 200.0f + ((float)mouse.x / (float)w) * 800.0f; }
      ...
      sprintf(
          buffer, " Audio Freq: %.2fHz (Move mouse X to change) ", frequency);
